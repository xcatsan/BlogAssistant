...
{	
	...
	NSRect backgroundRect = NSInsetRect(cellFrame, 2.5, 1.0);
	NSBezierPath* path =
		[NSBezierPath bezierPathWithRoundedRect:backgroundRect xRadius:5.0 yRadius:5.0];
	[[NSColor colorWithCalibratedRed:0 green:0 blue:0 alpha:0.25] set];
	[path fill];

	NSInteger controlState;
	
	for (CustomCellControl* cellControl in controls) {
		if (handlingControl == cellControl && handlingValue == [self objectValue]) {
			if (isMouseDown) {
